...
//        $request = cache($key, $value);//第三参数是时效期
    ...
    public static function getCurrentUid()
    {
        ...
        if($login_mode==1){
            ...
            if($state==0){
                return -1;
               // throw new BaseException(['msg'=>'账号未审核通过','errorCode'=>0]);
            }
            if($state==1){
                return -2;
               // throw new BaseException(['msg'=>'账号审核中','errorCode'=>1]);
            }
        }
        if($login_mode==2){
            if($user['invite_mobile']==''){
                throw new BaseException(['msg'=>'非邀请人员','data'=>999]);
            }
        }
