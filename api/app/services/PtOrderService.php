...
class PtOrderService
{
    ...
    public function CreatePtItemOrder($post, $uid)
    {
        ...
        try {
            ...
            Log::error('拼团订单');
            $pt = PtGoods::getPtGoods($post['goods_id']);
            if(!$pt['price']){
                throw new BaseException(['msg'=>'该拼团不存在']);}
            if(($goods_info['price']-$pt['price'])==($post['total_price']/$post['num'])){
                Log::error('拼团订单验证正确');
            }
            ...
            Db::commit();
            Goods::ReduceStock($res_order->toArray()['id']);//扣除库存
            return $data;
        } catch (\Exception $e) {
            Db::rollback();// 回滚事务
    ...
    public function setOrderData($post, $uid, $item, $goods)
    {
        ...
        $data['coupon_id'] = $post['coupon_id']; //优惠券
        ...
        if($data['coupon_id']>0){
            $data['order_money'] =  $data['order_money'] - $this->computeCouponMoney($data['coupon_id'],$uid,$post['json']);
        }
