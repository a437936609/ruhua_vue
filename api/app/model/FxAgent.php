...
class FxAgent extends BaseModel
{
    ...
    public static function add_fx_agent($ids)
    {

        Db::startTrans();
        try {
            for($i=0;$i<count($ids);$i++){
                $user=UserModel::where('id',$ids[$i])->find();
                if(!$user){
                   // throw new BaseException(['msg'=>'用户不存在']);
                    continue;
                }
                $fx=self::where('user_id',$ids[$i])->find();
                if ($fx){
                    //throw new BaseException(['msg'=>'该用户已是分销商']);
                    continue;
                }
                self::create(['user_id'=>$ids[$i],'invite_code'=>$user['invite_code']]);
