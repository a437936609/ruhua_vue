								...
								<div class="user_sear">
									<div class="sear_01">
										<div class="sear_01_01">
											...
											<el-input v-model="nickname" placeholder="请输入昵称" @keyup.enter.native="search(nickname)"></el-input>
								...
								<el-table :data="list" border style="width: 100%">
									...
									<el-table-column  label="昵称">
										<template slot-scope="scope">
											{{scope.row.nickname?scope.row.nickname:'未授权'}}
										</template>
									</el-table-column>
									...
									<el-table-column prop="vip" label="是否会员">
										<template slot-scope="scope">
											<a style="visibility: hidden">{{scope.row.vip == 1?scope.row.vip =true : scope.row.vip =false}}</a>
											<el-switch @change="set_hot(scope.row)" v-model="scope.row.vip" active-color="#409EFF"
									...
									<el-table-column prop="start" label="审核">
										<template slot-scope="scope">
											<el-switch
											  ...
											  :active-value="2"
											  :inactive-value="1"
											  @change="charge_user(scope.row.id,scope.row.start)">
											</el-switch>
										</template>
									</el-table-column>
...
<script>
	...
	export default {
		...
		methods: {
			charge_user(id,start){
				this.http.put_show('/user/admin/trial_user?uid='+id+'&start='+start).then(res=>{
					this._load()
				})
			},
			_load() {
			...
			set_hot(id) {
				var that = this;
				if(id['vip']){
					this.http.get('user/admin/set_vip_cms?id='+id['id']).then(res => {
						
					})
					console.log('11111111111')
				}else{
					this.http.get('user/admin/set_vip_cms_no?id='+id['id']).then(res => {
