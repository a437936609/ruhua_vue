			...
			<template v-for="(item,index) in list" v-key="index">
				<el-row class="inp" v-if="item.switch == 2">
					<el-form-item :label="item.desc">
						...
						<template v-if="item.key == 'product_classification'">
							<el-radio-group v-model="item.value">
								<el-tooltip placement="bottom">
									<div slot="content">直接从分类页就能看见产品</div>
									<el-radio label="0">分类页-产品</el-radio>
								</el-tooltip>
								<el-tooltip placement="right">
									<div slot="content">需点击二级类才能看见产品<br/></div>
									<el-radio label="1">二级类</el-radio>
								</el-tooltip>
							</el-radio-group>
						</template>
						...
						<template v-if="item.key == 'login_mode'">
							<el-radio-group v-model="item.value">
