		...
		<el-container>
			...
			<el-container>
				...
				<el-main style="background-color: #F3F3F3;">
					<el-button style="margin-bottom: 20px; display:flex;" size="small" type="primary" @click="add_hot">新增热门词</el-button>
					<el-table :data="tableData" stripe style="width: 100%">
						...
						<el-table-column prop="name" label="搜索次数">
...
<script>
	...
	export default {
		...
		methods: {
			get_hot_data() {
				this.http.get('search/recordcms').then(res => {
			...
			del(item) {
				...
				this.$confirm('确定删除该热门搜索词', '提示', {
					...
					this.http.put_show('search/admin/del_record?name=' + item.name).then(res => {
						this.$message({
							type: 'success',
							message: '删除成功!'
						});
