...
export default{
	...
	methods:{
		...
		get_data(id){
			...
			this.http.get('games/get_dzp_rule?id='+id).then(res=>{
				console.log(res)
		...
		customColorMethod(percentage) {
		  if (percentage < 30) {
			...
			{
				if(this.is_add){
					this.http.post('games/add_dzp_rule',this.form).then(res=>{
					...
					this.http.post('games/update_dzp_rule',this.form).then(res=>{
						if(res.data.status==200){
							this.$message({
								type: 'success',
								message: '添加成功!'
