					...
					<el-main >	
						...
						<div class="article" >
							<div class="tab-btn" >
								...
								<el-button @click="jump_worker" type="success" size="small">查看所有工人</el-button>
							</div>
							<template>
								<el-table :data="list" border style="width: 100%" >
									...
									<el-table-column label="状态" width="300px">
										<template slot-scope="scope">  
											...
											<span v-if="scope.row.status==1">待审查</span>
											<span v-if="scope.row.status==2">已通过</span>
										</template>
									</el-table-column>
									<el-table-column prop="operation" label="操作" width="300px">
										<template slot-scope="scope">
											<el-button type="danger" size="small" slot="reference" @click="del(scope.row.id)">删除</el-button>											 
											<a style="margin-left: 10px" v-if="scope.row.status==2" :href="url+scope.row.pdf_url" target="_blank"><el-button 
		...
		methods: {
			del(id) {
				this.$confirm('确定删除?', '提示', {
					...
					this.http.put_show('project/admin/del_worker_record?id='+id).then(res => {
						this.$message({
							type: 'success',
							message: '删除成功!'
						});
			...
			getCompany() {
				...
				// 	fullscreen: true
				// }); //显示加载 
				this.http.get('worker/get_worker_record?pid='+this.id).then((res) => {
