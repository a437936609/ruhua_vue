	...
	export default {
		data() {
			return {
				id:'',
				all:'',
				form:{nickname:'',goods_id:'',type:[],create_time:''},
				list: [],
				size: 10,
				total: '',
				options: [],
		...
		methods: {
			del(id) {
				this.$confirm('确定删除?', '提示', {
					...
					this.http.put_show('project/admin/del_store_record?id='+id).then(res => {
						this.$message({
							type: 'success',
							message: '删除成功!'
						});
			...
			getCompany() {
				...
				// 	fullscreen: true
				// }); //显示加载 
				this.http.get('project/admin/get_store_record?type=1&pid='+this.id).then((res) => {
