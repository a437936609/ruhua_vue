            ...
            <el-row :gutter="18">
              <template v-if="cate_id == 0">
                ...
                <el-col :span="6" v-for="(item, index) of img_list" :key="index">
                  <div
                    :class="tab_css(index)"
                    @click="choose_img(index)"
                    style="word-wrap: normal; word-break: break-all; padding: 5px; text-align: center;line-height: 30px;overflow: hidden;"
                  >
                    <template>
                  ...
                  <el-button
                    v-if="play_index!=index"
                    style="margin-left: 25%"
                    type="primary"
                    size="mini"
                    @click="play(item.url,index)"
                  >播放</el-button>
                  <el-button
                    v-else
                    style="margin-left: 25%"
  ...
  methods: {
    ...
    handleFileChange() {
      ...
      if (size / 1024 > 10) {
        ...
        this.http.post_show("video/admin/add_video", formData).then((res) => {
    ...
    del_img() {
      ...
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        this.http
          .post_show("video/admin/edit_video", {
            ids: ids,
          })
          .then((res) => {
            this.$message({
