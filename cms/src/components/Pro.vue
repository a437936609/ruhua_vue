	...
	export default {
		...
		methods: {
			...
			get_img(e) {
				...
				console.log('get_img_end:', e, this.forms.banner_imgs)
			},
			get_video(e) {
			...
			del_img(index) {
				...
				console.log('img',this.forms.banner_imgs)
			},
			get_delivery() {
			...
			check_data(){
				var that = this;
				if(that.forms.banner_imgs && !that.forms.banner_imgs[0]){
					this.$message({
						showClose: true,
						message: "未选择商品图片",
						type: "error"
