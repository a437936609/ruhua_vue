        ...
        <el-row>
          ...
          <el-col :span="5">
            ...
            <el-form-item label="选项" :label-width="formLabelWidth" v-if="!form[index].show">
              <el-input
                v-model="form[index].options"
                auto-complete="off"
                placeholder="用@@隔开,例:男@@女"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="5">
...
export default {
  ...
  methods: {
    ...
    sub() {
      ...
      this.http.post_show("wntable/add", this.sub_data).then(res => {
        //演示用虚拟接口
        if (res.status == 200) {
    ...
    select_type(index) {
      ...
      if (
        this.form[index].type == "radio" ||
        this.form[index].type == "check" ||
        this.form[index].type == "select"
      ) {
        this.show = false;
