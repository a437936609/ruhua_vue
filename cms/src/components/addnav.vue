<template>
	<div class="add-cate">
		<el-button type="primary" size="small" @click="addbox = true">添加导航</el-button>
		...
		<el-dialog title="" :visible.sync="addbox" width="35%" center @close="close">
			<el-form :model="addform">
				...
				<el-form-item label="选择文章" :label-width="formLabelWidth" v-if="addform.url_name == '文章'">
					<el-select v-model="addform.category" placeholder="请选择跳转的文章" @change="choose_article">
						<!-- <el-option label="顶级分类" value="0"></el-option> -->
						<el-option v-for="item in all_article" :value="item.id" :label="item.title"></el-option>
					</el-select>
				</el-form-item>
...
<script>
	...
	export default {
		...
		methods: {
			...
			choose_article(e){
				this.addform.url = '/pages/article/article?id='+e
			},
			close() {
				this.img_list = []
				this.addform = {
