	...
	<view class="imageUploadContainer">
		<view class="imageUploadList">
			<view class="imageItem" v-bind:key="index" v-for="(path,index) in value">
				<image :src="path.url" :class="{'dragging':isDragging(index)}" draggable="true" @tap="previewImage"
					:data-index="index" @touchstart="start" @touchmove="move" @touchend="stop"></image>
				<view v-if="isShowDel && $api.my('role_id')>3" class="imageDel" @tap="deleteImage" :data-index="index">x
				</view>
...
<script>
	...
	export default {
		...
		props: ['value', 'enableDel', 'enableAdd', 'enableDrag', 'serverUrl', 'formData', 'limit', 'fileKeyName',
			'showUploadProgress', 'serverUrlDeleteImage', 'names'
		],
		data() {
			return {
					...
					success: function(e) {
						...
						if (_self.limit) {
							...
							if (availableImageNumber < imagePathArr.length) {
								uni.showToast({
									title: '图片总数限制为' + _self.limit + '张，当前还可以选' +
										availableImageNumber + '张',
									icon: 'none',
									mask: false,
									duration: 2000
								});
									...
									uni.uploadFile({
										...
										success: function(res) {
											if (res.statusCode === 200) {
												_self.value[remoteUrlIndex] = res
													.data
												completeImages++
