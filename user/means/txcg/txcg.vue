<template>
	<view class="success">
		<view class="s_tit" v-if="list.status == 0">
			提现申请成功<span @click="jump">完成</span>
		</view>
		<view class="BH10"></view>
		<view class="chengg">
			<img :src="require('@/static/img/12.png')" v-if="list.status != -1"/>
			<img :src="require('@/static/img/txbh.png')" v-else/>
			<view class="cg" v-if="list.status == 0">等待审核中...</view>
			<view class="cg" v-if="list.status == 1">提现申请成功</view>
			<view class="cg" v-if="list.status == -1">申请已被驳回</view>
			<view class="cg_q">¥ {{list.money}}</view>
		</view>
		<view class="txfs">
			<view class="txfs_l">提现方式</view>
			<view class="txfs_r"><img :src="require('@/static/img/yin'+list.card_type+'.png')" v-if="list.card_type!=0" />{{list.bk_name}}（{{card_num}}）</view>
		</view>
		<view class="txzt">提现状态</view>
		<uni-steps :options="
			buzhou" direction="column" :active="step" active-color="#E1461D">
		</uni-steps>
		<view class="dzsj">
...
<script>
	...
	export default {
		...
		methods: {
			get_one_tx() {
				this.$api.http.get('fx/user/get_userRecordId?id=' + this.id).then(res => {
			...
			_load() {
				this.$api.http.get('fx/user/get_userRecordId?id=' + this.id).then(res => {
					...
					if(this.list.status == -1){
						this.three[0].desc = this.list.create_time
						this.buzhou = this.three
						this.step = 2
					}
				})
