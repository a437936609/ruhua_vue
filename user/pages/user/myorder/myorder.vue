<template>
	<view>
				...
				<view class="right">
					...
					<view class="price-box">
						...
						<button v-if="my_order.state == 1 && item.state < 2"
							style="height: 30px; line-height: 30px;margin-left: 30%;" type="default" size="mini"
		...
		<view class="btn" v-if="my_order.payment_state==1">
			...
			<!-- <view class="btn_2" v-if="my_order.state==1" @click="jump_tograde">评分</view> -->
			<button class="btn_1" v-if="my_order.state==0 && my_order.shipment_state==1"
				@click="jump_wuliu(my_order.order_id)">物流</button>
			<view class="btn_1" v-if="my_order.state==0 && my_order.shipment_state==1"
		...
		<view v-if="my_order.payment_state==1 && my_order.shipment_state<1">
			<view class="yanz"
				...
				<image v-if="my_order.qrcode" :src="my_order.qrcode"
		...
		methods: {
			...
			get_my_order() {
				...
				this.$api.http.get('order/user/get_order_one', {
					...
					console.log(res.data)
					if (res.data.payment_state == 1 && res.data.pt_state == 1) {
			...
			set_order_finish(id) {
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							console.log('用户点击确定');
							that.$api.http.post('order/user/receive', {
								id: id
							}).then(res => {
								if (res.data == 1) {
									that.$api.msg('操作成功')
			...
			jump_pindetail() {
				if (this.is_haoyou) {
					...
					uni.navigateTo({
						url: '/pages/invite/invite?id=' + this.my_order.item_id + '&img=' + img + '&price=' +
							price + '&name=' + name
					})
				} else {
					uni.navigateTo({
