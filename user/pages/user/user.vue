		...
		<view class="user-section">
			...
			<view class="vip-card-box">
				...
				<view class="tit" v-if="vip==0">
					...
					普通用户 <text style="margin-left: 40px;"
						v-if="userinfo">积分：{{userinfo.points?userinfo.points:0}}</text>
				</view>
				<view class="tit" v-else>
					<text class="yticon icon-iLinkapp-"></text>
					尊贵会员 <text style="margin-left: 40px;">积分：{{userinfo.points?userinfo.points:0}}</text>
				</view>
		...
		methods: {
			...
			bind(e) {
				...
				_self.$api.http.post('auth/bind_wx_mobile', obj).then(res => {
					console.log(res)
					if (res.data == 1) {
						cache_user.reset_storage().then(res => {
							console.log(res)
							_self.userinfo.points = res.points
