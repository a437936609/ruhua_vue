	...
	<view class="container">
		...
		<view class="tui-header-banner">
			<view class="tui-hot-search" :style="gundong?'margin-top: 70px':'margin-top: 40px'">
				<block  v-if="sys_switch && sys_switch.hot_swicth == 1">
		...
		<view class="tui-product-category">
			<swiper :indicator-dots="category && category.length > 4 ?true:false" :interval="5000" :duration="150" class="tui-banner-swiper"
			 style='height:92px' :circular="true" indicator-color="#ccc" indicator-active-color="rgba(251,88,106, 0.8)">
				...
				<swiper-item v-if="category && category.length>8">
					<view style="display: flex;flex-wrap: wrap;">
	...
	export default {
		...
		methods: {
			get_token(){
				...
				console.log('tokenx:',this.request_num,token)
				
				if(!token && this.request_num<1){
					console.log('走了if')
					setTimeout(()=>{
						this.get_token()
						this.request_num++ 
					...
					this.prmSwitch()
					this.check_switch()
					this.check_user_mode()
					this._load()
				}
			},
			async check_user_mode(){
				let user_mode = uni.getStorageSync('user_mode')
				if(user_mode){
					if(user_mode.code == 1){
						uni.reLaunch({
							url:'../../means/sub_chage/sub_chage'
						})
					}
					if(user_mode.code == 2){
						uni.reLaunch({
							url:'../../means/wait_chage/wait_chage'
						})
					}
				}else{
					setTimeout(()=>{
			...
			get_page_data(){
				this.$api.http.get('product/get_recent?type=hot&page='+this.page_num).then(res=>{
					if(res.data){
			...
			_load() {
				...
				let a = productModel.getProductHotRecent(this.page_num)
				...
				Promise.all([a, b, c, d]).then(res => {
					console.log('获取数据成功')
					for (let k in res[0].data) {
						let v= res[0].data[k]
						this.productList.push(v)
					}
					// this.productList.push(res[0].data) 
