...
<script>
	...
	export default {
		...
		onReachBottom() {
			...
			this.page_num++
			this.page_nums++
			console.log(this.is_cate_data)
			if(this.is_cate_data){
				this.get_pro()
				this.page_nums=0
			}
			else if(this.is_cate_datas){
				this.page_num=0
				this.get_pro_cate()
			}
			else{
		...
		methods: {
			...
			get_pro() {
				...
				productModel.getProList(this.page_num).then(res => { //所有商品
					if(res.data){
						this.is_cate_data = true
						if(this.page_num>0){
						...
						this.is_cate_data = false
					}
					this.is_cate_datas=false;
					// this.productList = res.data
					this.all = res.data
			...
			get_pro_cate() {
				...
				productModel.getProductCate(this.pro_cate_id,this.page_nums).then(res => {
					if(res.data.length>0){
						this.is_cate_datas=true;
						if(this.page_nums>0){
						for (let k in res.data) {
							let v = res.data[k]
							this.productList.push(v);
						...
						console.log("分类商品1:", this.productList)
					}else{
						this.page_nums=0;
						this.is_cate_datas=false;
