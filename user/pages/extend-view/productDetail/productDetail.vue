<template>
	<view class="container">
		...
		<!--底部操作栏-->
		<Bottom :opcity="opcity" :shop_car_num="shop_car_num" :collected="collected" :list="list" @collecting="collecting"
		 @showPopup="showPopup" @showPopupxx="showPopupxx"></Bottom>
		...
		<uni-popup ref="footerShare" type="share">
			<goods-share title="分享" @select="select" :list="list"></goods-share>
		</uni-popup>
		<!-- 生成海报 -->
	...
	export default {
		...
		methods: {
			...
			async get_pro() {
				...
				this.list = res.data
				this.goods=res.data
				console.log(this.goods,9989585)
				if (res.data.video) {
					this.is_auto = false
				} else {
					this.is_auto = true
			...
			showPopup: function(e, is_bottom_click) {
				console.log('xxxxx',e, is_bottom_click)
