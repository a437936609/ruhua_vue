...
use think\Exception;
use app\model\VipOrder as VopOrderModel;
...
class AppPayService
{
    ...
    public function payvip()
    {
        ...
        //$order_money = OrderModel::getOrderAttr($this->orderID, 'order_money');//获取订单指定字段
        $order_money=VipOrder::getOrderAttr($this->orderID,'pay_money');
        $order_money = 100 * $order_money;
        return $this->getAppPayVip($order_money);//进行微信支付
    }
    ...
    //获取vip app支付订单
    public function getAppPayVip($order_money)
    {
        $url = "https://api.mch.weixin.qq.com/pay/unifiedorder";
        $api_url = SysConfig::where(['key' => 'api_url'])->value('value');
        $onoce_str = $this->getRandChar(32);
        ...
        $data["mch_id"] = $this->config['mch_id'];
        $data["nonce_str"] = $onoce_str;
        $data["notify_url"] = $api_url . "/order/pay/appvipNotify";
        $data["out_trade_no"] = $this->orderNO;
        $data["spbill_create_ip"] = $this->get_client_ip();
        $data["total_fee"] = $order_money;
        $data["trade_type"] = "APP";
        ...
        $response = $this->xmlToArray($response);//将微信返回的结果xml转成数组
        if ($response['result_code'] == 'SUCCESS') {
            $this->recordPreVipOrder($response['prepay_id']);//写入prepay_id
            $res = $this->getOrder($response['prepay_id']);
            return json($res);
        } else {
            Log::channel('msgLog')->write('订单'.$this->orderNO."支付失败,原因：".$response['err_code_des']);
            return app('json')->fail($response['err_code_des']);
        }
    ...
    {
        $list=VipOrder::where('id',$this->orderID)->find();
        VipOrder::update(['order_status'=>2],['id'=>$this->orderID]);
    ...
    private function recordPreVipOrder($wxOrder)
    {
        VipOrder::where('id', $this->orderID)
            ->update(['prepay_id' => $wxOrder]);
    }
...
//    }
    ...
    private function checkVipValid()
    {
        $order = VipOrder::where('id', $this->orderID)->where('state',0)->find();
        if (!$order) {
            throw new BaseException(['msg' => '订单不存在']);
        }
        if (!TokenService::isValidOperate($order->user_id)) {
