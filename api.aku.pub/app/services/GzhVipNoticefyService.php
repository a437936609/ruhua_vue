...
class GzhVipNoticefyService extends WxPayNotify
{
    //继承覆盖
    ...
    public function NotifyProcess($objData, $config, &$msg)
    {
        ...
        $res=$notify->updatevip($data['out_trade_no']);
        if (!$res) {
            return false;
        }
        return true;
    ...
    {
    	VipOrderModel::where('id',$orderID)->update(['order_status'=>2,'pay_time'=>time()]);
